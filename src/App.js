...
function App() { 
  ...
  function addTodo(e) {
    ...
    setTodoList(prevTodoList => {
      return [...prevTodoList, {id: tempId, listId: selected.id, name: name, complete: false}]
    })
    //we also want to add this to the list that is on display 
    if (name === '') return
    setTodoListDisplay(prevTodoListDisplay => {
      return [...prevTodoListDisplay, {id: tempId, listId: selected.id, name: name, complete: false}]
    })
    //clear out the input field
  ...
  function clearTodo(e) {
    ...
    const newTodoListDisplay = todoListDisplay.filter(todo => !todo.complete)
  ...
  function toggleTodo(id) {
    ...
    if(todo){
      ...
      setTodoList(newTodoList) 
      const newTodoListDisplay = newTodoList.filter(todoList => todoList.listId === selected.id)
      setTodoListDisplay(newTodoListDisplay)
    } 
  }
